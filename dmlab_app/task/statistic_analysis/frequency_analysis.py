...
class FrequencyAnalysis(Base):
    ...
    def _check_valid_params(self, logger, params=None):
        if not params:
            ...
            return False
        elif not params.get('selected_column'):
            logger.exception('params has no attribute name "selected_column"')
            return False
        else:
            return True
        ...
        if success:
            ...
            if evaluation_file:
                try:
                    ...
                    data = self._frequency_analysis(data_content, params['selected_column'])
    ...
    def _frequency_analysis(self, data, selected_column):
        ...
        for row in data['content']:
            if mp.get(row[selected_idx]):
                mp[row[selected_idx]] += 1
            else:
                mp[row[selected_idx]] = 1
        sorted_mp = sorted(mp.items(), key=operator.itemgetter(1), reverse=True)
        rsts['headers'] = [selected_column, 'Count']
        contents = []
        for item in sorted_mp:
            content = [item[0], item[1]]
            contents.append(content)
        rsts['content'] = contents
        return rsts
