...
class FeatureStructure(Base):
    ...
    def _check_valid_params(self, logger, params=None):
        if not params:
            ...
            return False
        elif not params.get('new_feature'):
            logger.exception('params has no attribute name "new_feature"')
            return False
        elif not params.get('expression'):
            logger.exception('params has no attribute name "expression"')
            return False
        else:
            return True
    ...
    def execute(self, evaluation_id=None, params=None, item_id=None):
        ...
        if success:
            ...
            if evaluation_file:
                ...
                rsts = self._feature_structure(data_content, selected_columns, new_feature, expression)
    ...
    def _feature_structure(self, data, selected_columns, new_feature, expression):
        ...
        for idx, item in enumerate(data['content']):
            ...
            for idx1, item1 in enumerate(item):
                if idx1 in selected_columns_idx:
                    rst.append(item1)
                    rst_str = rst_str.replace('{'+columns[idx1]+'}', item1)
            ans = eval(rst_str)
            rst.append(ans)
            rsts.append(rst)
