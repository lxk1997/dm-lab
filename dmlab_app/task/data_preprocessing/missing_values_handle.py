...
from ...utils import numeric, NpEncoder
...
class MissingValuesHandle(Base):
    component_id = 1
    ...
    def _check_valid_params(self, logger, params=None):
        if not params:
            ...
            return False
        elif not params.get('solve_method'):
            logger.exception('params has no attribute name "solve_method"')
            return False
        else:
            return True
        ...
        if success:
            try:
                ...
                if evaluation_file:
                    ...
                    columns = []
                    for idx, item in enumerate(data_content['headers']):
                        if item in selected_columns:
                            selected_columns_idx.append(idx)
                            columns.append(item)
                    rsts = []
                    for idx, item in enumerate(data_content['content']):
                        rst = []
                        for idx1, item1 in enumerate(item):
                            if idx1 in selected_columns_idx:
                                if item1 == '':
                                    item1 = np.nan
                                rst.append(numeric(item1))
                        rsts.append(rst)
                    print(rsts)
                    if solve_method not in ['drop']:
                        print(solve_method)
                        imp_mean = SimpleImputer(missing_values=np.nan, strategy=solve_method)
                        try:
                            imp_mean.fit(rsts)
                        except Exception as e:
                            print(e.args)
                        rsts = imp_mean.transform(rsts)
                    else:
                        results = []
                        for rst in rsts:
                            flag = False
                    ...
                    with fs.open(data_path, 'w') as fout:
                        json.dump(rsts, fout, indent=2, ensure_ascii=False, cls=NpEncoder)
                else:
