...
class LRClassifier(Base):
            ...
            if evaluation_file:
                ...
                try:
                    ...
                    feature_train, feature_test, target_train, target_test = train_test_split(feature_rsts['content'],
                                                                                              ...
                                                                                              test_size=0.33,
                                                                                              random_state=42)
                    dt_model = linear_model.LogisticRegression(penalty=numeric(params['penalty']),
                                                               dual=numeric(params['dual']),
                                                               tol=numeric(params['tol']),
                                                               fit_intercept=numeric(params['fit_intercept']),
                                                               intercept_scaling=numeric(params['intercept_scaling']),
                                                               class_weight=numeric(params['class_weight']),
                                                               solver=numeric(params['solver']),
                                                               C=numeric(params['C']),
                                                               multi_class=numeric(params['multi_class']),
                                                               max_iter=numeric(params['max_iter']),
                                                               l1_ratio=numeric(params['l1_ratio']))
    ...
    def _get_evaluation_report(self, item_id, limit=None, offset=None):
        ...
        if report:
            ...
            datau.set_content([[round(json_report['acc'], 4), round(json_report['pre'], 4),
                                round(json_report['rec'], 4), round(json_report['f1_score'], 4),
                                json_report['num_gt']]])
            reports.append({
                'id': cur,
                'name': 'Total',
    ...
    def get_score(self, item_id):
        ...
        if evaluation_file:
            ...
            json_report = json.loads(report_content)
            score_content = 'Acc: %s,AP: %s, AR: %s' % (
            round(json_report['acc'], 2), round(json_report['pre'], 2), round(json_report['rec'], 2))
        ...
        if not score_field or not item_id:
            ...
            if evaluation_file:
                ...
                if json_report.get(target):
                    ...
                    finally_score = round((cur_score * score_field['result_ratio'] / 100.0 + (
                                (cur_score * (100 - score_field['result_ratio']) / 100.0) ** (time_value))) * (
                                                      (score_field['count_ratio'] / 100) ** (cnt + 1)), 2)
