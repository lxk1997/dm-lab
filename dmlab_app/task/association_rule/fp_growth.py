...
class FPGrowth(Base):
        ...
        if success:
            ...
            if evaluation_file:
                ...
                try:
                    ...
                    itemsets, rules = self._fp_growth(transactions=content['content'],
    ...
    def _get_evaluation_report(self, item_id, limit=None, offset=None):
        ...
        if evaluation_file:
            ...
            datau = DatasetUtils()
            datau.set_header(['Min Support', 'Min Confidence'])
            datau.set_content([[round(json_report['min_support'], 4), round(json_report['min_confidence'], 4)]])
            reports.append({
                'id': cur,
                'name': 'Parameter',
                'type': 'table',
            ...
            datau.set_header(
                ['Antecedent', 'Consequent', 'Confidence', 'Support', 'Lift', 'conviction', 'rpf', 'Count'])
            rules = json_report['rules']
            contents = []
            for rule in rules:
                content = ['%s(%s)' % (','.join(list(rule['lhs'])), rule['count_lhs']),
                           '%s(%s)' % (','.join(list(rule['rhs'])), rule['count_rhs']), round(rule['conf'], 4),
                           round(rule['supp'], 4), round(rule['lift'], 4), round(rule['conv'], 4),
                           round(rule['rpf'], 6), rule['count_full']]
    ...
    def _fp_growth(self, transactions, min_support=0.5, min_confidence=0.5):
        support_threshold = len(transactions) * min_support
        patterns = pyfpgrowth.find_frequent_patterns(transactions, support_threshold=support_threshold)
        itemsets = {}
        for key in patterns.keys():
            count = len(list(key))
            if itemsets.get(count):
                itemsets[count][','.join(list(key))] = patterns[key]
            else:
                itemsets[count] = {','.join(list(key)): patterns[key]}
        ...
        for rule in rules.keys():
            lhs = list(rule)
            rhs = list(list(rules[rule])[0])
            conf = list(rules[rule])[1]
            supp = 0
            supp_a = 0
            supp_b = 0
            for transaction in transactions:
                if set(lhs + rhs).issubset(transaction):
                    supp += 1
                if set(rhs).issubset(transaction):
                    supp_b += 1
                if set(lhs).issubset(transaction):
                    supp_a += 1
            _supp = supp * 1.0 / len(transactions)
            _supp_b = supp_b * 1.0 / len(transactions)
            lift = conf / _supp_b
            conv = (1 - _supp_b) / (1 - conf)
            count_full = supp
            count_lhs = supp_a
            count_rhs = supp_b
            item = {
