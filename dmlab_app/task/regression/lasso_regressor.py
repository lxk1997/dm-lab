            ...
            if evaluation_file:
                ...
                try:
                    ...
                    feature_train, feature_test, target_train, target_test = train_test_split(feature_rsts['content'],
                                                                                              ...
                                                                                              test_size=0.33,
                                                                                              random_state=42)
                    dt_model = Lasso(alpha=numeric(params['alpha']),
                                     fit_intercept=numeric(params['fit_intercept']),
                                     max_iter=numeric(params['max_iter']),
                                     normalize=numeric(params['normalize']),
                                     precompute=numeric(params['precompute']),
                                     tol=numeric(params['tol']),
                                     warm_start=numeric(params['warm_start']),
                                     positive=numeric(params['positive']),
                                     selection=numeric(params['selection']),
