...
@login_required
def handle_get_public_datasets():
    limit = request.args.get('limit', None)
    offset = request.args.get('offset', None)
    datasets = Dataset().query(school_id='000000', limit=limit, offset=offset)
    data = {
        'detail': datasets,
    }
    msg = 'ok'
...
def handle_copy_dataset():
    ...
    if datasets:
        ...
        datasets = Dataset().query(school_id='000000', dataset_id=dataset_id)
        if not datasets:
            msg = 'Dataset %s does not exists.' % dataset_id
            error = 1
        else:
            if user_only:
                dataset_id = Dataset().create(dataset_name=dataset_name,
                                              experimental_item_id=experimental_item['experimental_item_id'],
                                              description=description, file_key=datasets[0]['file_key'], user_only=1,
                                              user_id=g.user['user_id'])
            else:
                dataset_id = Dataset().create(dataset_name=dataset_name,
                                              experimental_item_id=experimental_item['experimental_item_id'],
                                              description=description, file_key=datasets[0]['file_key'])
            if dataset_id != -1:
                msg = 'ok'
                error = 0
                data = Dataset().query(dataset_id=dataset_id)
