...
class NBClassifier extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            dataset_columns: [],
            ...
            <div>
                <Collapse defaultActiveKey={['1']} accordion expandIconPosition={'right'} style={{'margin-bottom': '45px'}}>
                    ...
                    <Panel header="基础参数" key="2">
                        <div>分布</div>
                        <Select style={{ width: 130 }} defaultValue={this.state.method} onChange={this.onMethodChange}>
                            <Option value={"Gaussian"}>{"高斯分布"}</Option>
                            <Option value={"Multinomial"}>{"多项式分布"}</Option>
                            <Option value={"Bernoulli"}>{"二项分布"}</Option>
                        </Select>
                    </Panel>
                </Collapse>
...
export default withPropsAPI(NBClassifier);
